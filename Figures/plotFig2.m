...
tmp.LR_A = f_regImages(Fig2.LR_A,refParcellation,settings.parcellation,0).*BM;
tmp.LR_B = f_regImages(Fig2.LR_B,refParcellation,settings.parcellation,0).*BM;
tmp.IRFx2_A = f_regImages(Fig2.IRFx2_A,refParcellation,settings.parcellation,0).*BM;
tmp.IRFx2_B = f_regImages(Fig2.IRFx2_B,refParcellation,settings.parcellation,0).*BM;
...
f = figure;
f_plotBar(barData,colors=cmpvir,legend={'tA','tB'},ylabel='r',title='Timing Coefficients');

%% Fig 2 D

f = figure;
f_plotMap(mean(subAvg.Fig2.LR_perf,3,'omitnan').*plotBM,cmp=cmpvir,bounds=[0 1],title='LR Performance',clabel='r');
for i = 1:12
    f_plotAllenRegion(i,2,linewidth=2,color=[0 0 0]);
end
...
f = figure;
f_plotMap((mean(subAvg.Fig2.LR_perf,3,'omitnan')-mean(subAvg.Fig1.inv_perf,3,'omitnan')).*plotBM,cmp=cmpbbr,bounds=[-1 1],title='LR vs. Global IRF',clabel='\Deltar');
for i = 1:12
    f_plotAllenRegion(i,2,linewidth=2,color=[0 0 0]);
end
...
f = figure;
f_plotMap(mean(subAvg.Fig2.IRFx2_perf,3,'omitnan').*plotBM,cmp=cmpvir,bounds=[0 1],title='IRFx2 Performance',clabel='r');
for i = 1:12
    f_plotAllenRegion(i,2,linewidth=2,color=[0 0 0]);
end
...
f = figure;
f_plotMap((mean(subAvg.Fig2.IRFx2_perf,3,'omitnan')-mean(subAvg.Fig1.inv_perf,3,'omitnan')).*plotBM,cmp=cmpbbr,bounds=[-1 1],title='IRFx2 vs. Global IRF',clabel='\Deltar');
for i = 1:12
    f_plotAllenRegion(i,2,linewidth=2,color=[0 0 0]);
end
...
barData{4} = squeeze(mean(subAvg.Fig2.LR_perf.*plotBM,[1,2],'omitnan'));
barData{5} = squeeze(mean(subAvg.Fig2.IRFx2_perf.*plotBM,[1,2],'omitnan'));
...
f = figure;
f_plotBar(barData,colors=cmpvir,legend={'Invariant','SSp','Variant','LR','IRFx2','shuffled LR','shuffled IRFx2'},ylabel='r',title='Model Performance Comparison');
